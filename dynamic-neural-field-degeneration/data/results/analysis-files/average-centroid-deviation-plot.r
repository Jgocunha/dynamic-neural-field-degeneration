...
plot_centroid_avg_deviation <- function(deviations, locationDirPath, figTitle, subTitle, 
  ...
  # Plot the average deviation with shaded error margins
  p <- ggplot(plot_data, aes(x = Iterations, y = Avg)) +
    ...
    theme(
      ...
      plot.subtitle = element_text(size = 14, face = "italic", color = "gray30"),
...
generate_combined_plot <- function(deviations_list, targetCentroids, figTitle, experimentName) {
  ...
  # Create a dummy plot for the legend (using the first plot style)
  legend_plot <- ggplot(data = data.frame(x = 1:10, y = 1:10), aes(x = x, y = y)) +
    geom_line(color = '#B3CDE0', size = 1.5) +
    geom_ribbon(aes(ymin = y - 0.2, ymax = y + 0.2), fill = "#B7E4D9", alpha = 0.5) +
    labs(y = "Absolute Deviation", x = "Simulation Iterations") +
    theme_void() + theme(legend.position = "none")
  
  legend_grob <- ggplotGrob(legend_plot)  # Extract legend grob
  ...
  combined_plot <- arrangeGrob(
    ...
    legend_grob,                               # Add custom legend in empty space
    layout_matrix = rbind(c(1, 1),
                          c(1, 1),
                          c(NA, 2))  # Custom layout for the legend
  )
