	...
	namespace element
	{
			...
			const std::vector<double>& input, const std::vector<double>& targetOutput, double learningRate) const
		{
			...
			for (size_t i = 0; i < inputSize; ++i) {
				for (size_t j = 0; j < outputSize; ++j)
				{
					if(degeneracyType != ElementDegeneracyType::WEIGHTS_DEACTIVATE)
					{
						log(DEBUG, "Element degeneracy type is NOT weights deactivate!\n");
						weights[i][j] += learningRate * (error[j] - eta * weights[i][j]) * input[i];
					}
					else
					{
						log(DEBUG, "Element degeneracy type IS weights deactivate!\n");
