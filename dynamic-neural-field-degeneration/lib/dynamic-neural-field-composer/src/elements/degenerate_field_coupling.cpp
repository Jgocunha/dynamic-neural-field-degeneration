...
{
	DegenerateFieldCoupling::DegenerateFieldCoupling(const std::string& id, int outputSize, const element::FieldCouplingParameters& parameters)
		: FieldCoupling(id, outputSize, parameters)
	{
		// Assert that the sizes are positive
		assert(outputSize > 0 && parameters.inputFieldSize > 0);
